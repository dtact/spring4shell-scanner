		...
		if p, err := func() (bool, error) {
			...
			if f.FileInfo().IsDir() {
				...
				if !strings.EqualFold(parts[0], "CachedIntrospectionResults") {
					// not CachedIntrospectionResults
					...
					if !b.IsAllowList(h) {
						...
						fmt.Fprintln(b.writer.Bypass(), color.RedString("[!][%s] found CachedIntrospectionResults class file with hash %x (version: %s) \u001b[0K", strings.Join(append(w, f.Name()), " -> "), h, version))
